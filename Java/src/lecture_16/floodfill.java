...
public class floodfill {
	...
	public static void main(String[] args) {
		int[][] mat = { { 1, 0, 1, 0 }, { 1, 1, 1, 0 }, { 0, 1, 0, 1 }, { 0, 0, 1, 1 } };
	...
	private static boolean isPath(int[][] mat, int row, int col, boolean[][] visited) {
		if (row >= mat.length || col >= mat.length || row<0 || col<0 || mat[row][col] == 0 
		...
		visited[row][col] = true;
		boolean isLeftPath = isPath(mat, row, col-1, visited);
		boolean isRightPath = isPath(mat, row, col+1, visited);
